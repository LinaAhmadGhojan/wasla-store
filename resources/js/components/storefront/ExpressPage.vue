  ...
  <div class="express-page" dir="rtl">
    ...
    <div class="shell">
      <div class="mode-switch" role="tablist" aria-label="اختر نوع التسوق">
        ...
        <a href="/express" class="mode-btn on" role="tab" aria-current="page" @click="rememberChannel('express')">وصلة السريعة</a>
      </div>
      ...
      <div class="layout">
        ...
        <div class="results">
          ...
          <p class="note">البيانات من وصلة السريعة — الطلب الحقيقي قيد التجهيز.</p>
        </div>
      </div>
    </div>
...
async function fetchBrowse() {
  ...
  try {
    ...
    loadError.value = e?.response?.data?.message || 'تعذّر تحميل وصلة السريعة. جرّبي لاحقاً.';
    expressStores.value = [];
    expressFoods.value = [];
  } finally {
    loading.value = false;
